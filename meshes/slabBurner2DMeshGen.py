...
# define the experimental chamber points. This makes the slab centered horizontally
lowerLeft = convert_to_point((0.0, 0.0))
upperLeft = convert_to_point((0.0, heightMultiplier*slabHeight))
lowerRight = convert_to_point((2*beginSlab + slabLength, 0.0))
upperRight = convert_to_point((2*beginSlab + slabLength, heightMultiplier*slabHeight))

# define a list of points for the slab burner, starting with the left most point
slabBoundaryLocations = [(beginSlab, 0),
                         (beginSlab + slabHeight, slabHeight),
                         (beginSlab + slabLength, slabHeight),
                         (beginSlab + slabLength, 0.000)]
...
define_boundary([
    slabBoundary[0:2],   # Ramp up: starts at (0.02, 0), ends at (0.030, 0.010)
    slabBoundary[1:3],  # Flat top: starts at (0.030, 0.010), ends at (0.100, 0.010)
    slabBoundary[2:4]   # Vertical drop: starts at (0.100, 0.010), ends at (0.100, 0.000)
], "slab", boundary_ids)
