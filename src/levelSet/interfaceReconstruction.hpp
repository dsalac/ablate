...
//      Vec cellVec[2] = {nullptr, nullptr}, cellGradVec[2] = {nullptr, nullptr};  // Cell-based data
      ...
      void SetMasks(DM vofDM, Vec vofVec, const ablate::domain::Field vofField, const PetscInt nLevels, PetscInt *cellMask, PetscInt *vertMask);

      void SmoothVOF(DM vofDM, Vec vofVec, const PetscInt vofID, DM smoothVOFDM, Vec smoothVOFVec[2], const PetscInt* subpointIndices);

      void InitalizeLevelSet(DM vofDM, Vec vofVec, const ablate::domain::Field vofField, const PetscInt *cellMask, const PetscInt *vertMask, Vec lsVec[2], PetscReal *closestPoint, PetscInt *cpCell);
      ...
      PetscInt FMM_VertexBased_GG_2D(const PetscInt v, const PetscInt *vertMask, PetscScalar *updated, PetscReal *phi[2]);
      PetscInt CheckSolution(const PetscInt vert, const PetscInt nVert, const PetscInt *verts, const PetscInt *vertMask, PetscReal phi[2], PetscReal *lsArray[2]);
...
//      PetscScalar LScircle(const PetscReal* x, const PetscInt dim);
      ...
      void arbit_interface(DM dm, const ablate::domain::Field levelSetField, Vec auxVec, std::string shapeName);
