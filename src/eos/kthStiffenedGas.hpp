#ifndef ABLATELIBRARY_KTHSTIFFENEDGAS_HPP
#define ABLATELIBRARY_KTHSTIFFENEDGAS_HPP
...
class KthStiffenedGas : public EOS {
   private:
    ...
    struct FunctionContext {
        ...
        Parameters parameters;
        const std::vector<domain::Field>* fields;  // Add fields to context
    };

    // Helper function to compute total density from ALPHAKRHOK
    static PetscErrorCode ComputeTotalDensity(const PetscReal conserved[], const std::vector<domain::Field>& fields, PetscReal* density);
   ...
   public:
    explicit KthStiffenedGas(std::shared_ptr<ablate::parameters::Parameters>, std::vector<std::string> species = {});
